...
Calculate Pearson correlation of gene expression values (TPM) for mock community (filtered for MC5 and MC19 species)
...
TPM calculations for mock communities
...
MC_TPM_pearson_fmt <- MC_TPM_pearson_df_filt %>% dplyr::select(comparison, value)

MC_TPM_pearson_fmt$metric <- "Pearson_correlation_gene_expression"

MC_combined_metrics <- rbind(MC_BC_dist_fmt, MC_TPM_pearson_fmt)


MC_combined_metrics$metric <- factor(MC_combined_metrics$metric, levels = c("sorensen_similarity",
                                                                     "Pearson_correlation_gene_expression"))
