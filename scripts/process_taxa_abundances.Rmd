...
mgx_alt_process_taxa_count_list <- lapply(mgx_to_pull, function(x){
  ...
  ###df with minimizers for mgx data, non-contaminant species that passed filters. No unclassified or human reads
  mgx_alt_process_minimizer_df <- mgx_alt_process_k2_minimizer_renorm[[mgx_id]] %>% 
    dplyr::select(k2_taxon, mgx_LIBID, paired_counts,rel_abun) %>%
    dplyr::rename(mgx_alt_paired_counts=paired_counts, 
                  mgx_alt_rel_abun = rel_abun)
  
  
  #need to tweak this again..
  #Species level analysis: Add unpaired counts and RPK at mgx level
  mgx_taxa_df <- mgx_taxa_count_summary_subsp_collapse[[mgx_id]] %>% 
    dplyr::rename(mgx_counts=read_count_sum,mgx_RPK=RPK_sum) %>% ungroup() %>%
    dplyr::filter(k2_taxon %in% minimizer_df$k2_taxon) %>% dplyr::select(-LIBID)
  ...
  mgx_alt_process_df <- merge(minimizer_df,mtx_minimizer_df, all=TRUE, by = "k2_taxon")
  
 
  mgx_alt_process_df[is.na(mgx_alt_process_df)] <- 0
  
  mgx_alt_process_df$mtx_LIBID <- x
  ...
  mtx_RPK_sum <- mgx_alt_process_df %>% pull(mtx_RPK) %>% sum(.)
  
  mtx_scaling_factor <- mtx_RPK_sum / 1000000
    
  mgx_RPK_sum <- mgx_alt_process_df %>% pull(mgx_RPK) %>% sum(.)
  
  mgx_scaling_factor <- mgx_RPK_sum / 1000000
  
  
  mgx_alt_process_df$mtx_TPM <- mgx_alt_process_df$mtx_RPK/mtx_scaling_factor
  mgx_alt_process_df$mgx_CPM <- mgx_alt_process_df$mgx_RPK/mgx_scaling_factor
  
  
  mgx_alt_process_df$transcriptional_activity <- mgx_alt_process_df$mtx_TPM/mgx_alt_process_df$mgx_CPM
  ...
  mtx_RPK_prok_sum <- mgx_alt_process_df %>% 
    dplyr::filter(str_detect(string=.$k2_taxon,
                             pattern="Malassezia",
                             negate=TRUE)) %>% pull(mtx_RPK) %>% sum(.)
  ...
  mgx_RPK_prok_sum <- mgx_alt_process_df %>% 
    dplyr::filter(str_detect(string=.$k2_taxon,
                             pattern="Malassezia",
                             negate=TRUE)) %>% pull(mgx_RPK) %>% sum(.)
  ...
  mgx_alt_process_df$mtx_TPM_prok <- ifelse(str_detect(string=mgx_alt_process_df$k2_taxon,
                             pattern="Malassezia",
                             negate=TRUE), mgx_alt_process_df$mtx_RPK/mtx_prok_scaling_factor,0)
  
  mgx_alt_process_df$mgx_CPM_prok <- ifelse(str_detect(string=mgx_alt_process_df$k2_taxon,
                             pattern="Malassezia",
                             negate=TRUE), mgx_alt_process_df$mgx_RPK/mgx_prok_scaling_factor,0)
  
  
  mgx_alt_process_df$prok_transcriptional_activity <- ifelse(str_detect(string=mgx_alt_process_df$k2_taxon,
                             pattern="Malassezia",
                             negate=TRUE),mgx_alt_process_df$mtx_TPM_prok/mgx_alt_process_df$mgx_CPM_prok, 0)
  
  

  return(mgx_alt_process_df)
  
})

names(mgx_alt_process_taxa_count_list) <- mtx_to_pull
