...
longitudinal_mtx_pangene_features <- lapply(longitudinal_metadata_passed$LIBID, function(x){
  ...
  #Load pangenes associated with contaminants from genera_to_remove and species_to_remove
  pangene_contams <- read_tsv(paste0("../data/longitudinal_cohort/MTX_annotations/",x,"_all_aligned_contam_summary.tsv"), 
                              show_col_types = FALSE, col_names=c("unpaired_read_count", "pangene", "k2_taxon", "uniref90_ID")) %>%
    dplyr::select(unpaired_read_count, pangene) %>% dplyr::filter(pangene != "-")
  
  pangene_contams_summed <- pangene_contams %>% group_by(pangene) %>% summarise(contam_read_count_sum=sum(unpaired_read_count))
  
  output <- merge(output, pangene_contams_summed, by = "pangene", all.x=TRUE)
  
  output[is.na(output)] <- 0
  
  #minus contaminants
  output$unpaired_read_count <- output$unpaired_read_count - output$contam_read_count_sum
  
  output$RPK <- output$unpaired_read_count / output$length
  
  return(output %>% dplyr::select(-contam_read_count_sum))
  })
...
longitudinal_mtx_uniref90_features <- lapply(longitudinal_metadata_passed$LIBID, function(x){
  ...
  #Load uniref90 IDs associated with contaminants from genera_to_remove and species_to_remove
  uniref_contams <- read_tsv(paste0("../data/longitudinal_cohort/MTX_annotations/",x,"_all_aligned_contam_summary.tsv"), 
                              show_col_types = FALSE, col_names=c("unpaired_read_count", "pangene", "k2_taxon", "uniref90_ID")) %>% 
    dplyr::filter(pangene == "-") %>%
    dplyr::select(unpaired_read_count, uniref90_ID)
  
  uniref_contams_summed <- uniref_contams %>% group_by(uniref90_ID) %>% summarise(contam_read_count_sum=sum(unpaired_read_count))
  
  output <- merge(output, uniref_contams_summed, by = "uniref90_ID", all.x=TRUE)
...
tech_reps_mtx_pangene_features <- lapply(tech_reps_metadata$LIBID, function(x){
  output <- read_tsv(paste0("../data/technical_repeats/MTX_annotations/",x,"_merged_panalign_annot.tsv"), show_col_types = FALSE) %>%
            dplyr::select(pangene, length, uniref90_ID, unpaired_read_count, 
                          emapper_max_annot_OG, emapper_OG, eggnog_cat, pangene_desc, eggnog_desc, uniref90_desc)
  ...
  #Load pangenes associated with contaminants from genera_to_remove and species_to_remove
  pangene_contams <- read_tsv(paste0("../data/technical_repeats/MTX_annotations/",x,"_all_aligned_contam_summary.tsv"), 
                              show_col_types = FALSE, col_names=c("unpaired_read_count", "pangene", "k2_taxon", "uniref90_ID")) %>%
    dplyr::select(unpaired_read_count, pangene) %>% dplyr::filter(pangene != "-")
...
tech_reps_mtx_uniref90_features <- lapply(tech_reps_metadata$LIBID, function(x){
  output <- read_tsv(paste0("../data/technical_repeats/MTX_annotations/",x,"_merged_transl-search_annot.tsv"), show_col_types = FALSE) %>%
            dplyr::select(uniref90_ID,AA_length,unpaired_read_count, emapper_max_annot_OG, 
                          emapper_OG, eggnog_cat, uniref90_desc,eggnog_desc)
  ...
  #Load uniref90 IDs associated with contaminants from genera_to_remove and species_to_remove
  uniref_contams <- read_tsv(paste0("../data/technical_repeats/MTX_annotations/",x,"_all_aligned_contam_summary.tsv"), 
                              show_col_types = FALSE, col_names=c("unpaired_read_count", "pangene", "k2_taxon", "uniref90_ID")) %>% 
    dplyr::filter(pangene == "-") %>%
    dplyr::select(unpaired_read_count, uniref90_ID)
...
write_tsv(longitudinal_OG_counts, "../data/longitudinal_OG_counts.tsv")

write_tsv(tech_reps_OG_counts, "../data/tech_reps_OG_counts.tsv")
...
median(within_individuals_longitudinal_OG_TPM_pearson_df$value) #0.8977749
...
median(betw_individuals_longitudinal_OG_TPM_pearson_df$value) #0.5229123
...
median(tech_reps_OG_TPM_pearson_df$value) #0.9902457
...
Boxplot showing the number of metatranscriptomic reads before computational removal of host and rRNA signals (red), number of non-human, non-rRNA reads after computational filtering (green) and number of microbial reads (comprising bacteria, viruses, fungi and archaea) after de-duplication and accounting for contaminant genera and species (blue). 

```{r}

#contaminant read pairs information from kraken2. This is all downstream of de-duplication

#see process_taxa_abundances.Rmd
mtx_k2_contam_read_pairs <- read_tsv("../data/mtx_k2_contaminant_read_pairs.tsv", show_col_types = FALSE) %>% dplyr::rename(LIBID=mtx_LIBID)
mtx_k2_contam_read_pairs$cat <- "non_contam_microbe_count"
mtx_k2_contam_read_pairs <- merge(mtx_k2_contam_read_pairs, mtx_stats_chosen %>% dplyr::select(LIBID, region, MICROBE_COUNT), by = "LIBID")
mtx_k2_contam_read_pairs$non_contaminant_microbial_read_pairs <- mtx_k2_contam_read_pairs$MICROBE_COUNT - mtx_k2_contam_read_pairs$estimated_contaminant_read_pairs

#Typically, >1Gbp of de-duplicated non-rRNA sequencing data (median of 3.7 million read pairs) was generated per library similar to other metatranscriptomic studies
median(mtx_k2_contam_read_pairs$non_contaminant_microbial_read_pairs)
...
mtx_stats_for_plot <- do.call("rbind",
           ...
           cat="after_rm"),
           data.frame(read_pairs=mtx_k2_contam_read_pairs$non_contaminant_microbial_read_pairs, region=mtx_k2_contam_read_pairs$region,
           cat=mtx_k2_contam_read_pairs$cat)
                                   ))



mtx_read_count_stats <- ggplot(mtx_stats_for_plot %>% mutate(region= fct_relevel(region, "Sc", "Ch", "Ac", "Vf", "Tw"),
                                     cat=fct_relevel(cat, "before_rm", "after_rm", "non_contam_microbe_count")), 
...
What percent reads mapped to human transcriptomes and were thus removed?

```{r}

mtx_stats_chosen$percent_reads_map_to_human <- (1- (mtx_stats_chosen$AFT_HUMAN_RM / mtx_stats_chosen$AFTER_FASTP))*100

median(mtx_stats_chosen$percent_reads_map_to_human)

```

#Additional analysis

Reproducibility analysis for microbial mock communities (MC).
...
MC5_combs <- MC_library_metadata %>% 
  dplyr::filter(str_detect(pattern = "MC5", string=.$sample))

MC5_combs <- combn(MC5_combs$LIBID, 2, simplify = FALSE) 
MC5_combs <- lapply(MC5_combs, function(x){ output <- stringi::stri_flatten(x, collapse="-")}) %>% unlist(.)


#For mock community B  (MC19)
MC19_combs <- MC_library_metadata %>% 
  dplyr::filter(str_detect(pattern = "MC19", string=.$sample))

MC19_combs <- combn(MC19_combs$LIBID, 2, simplify = FALSE) 
MC19_combs <- lapply(MC19_combs, function(x){ output <- stringi::stri_flatten(x, collapse="-")}) %>% unlist(.)

```

What are the species (true-positives) in the different mock comms?
...
Calculate Bray-Curtis distances on total sum scaled (TSS) relative abundances at species level for the mock communities 
...
#The distance matrix contains the permutations. We will subset for the unique and relevant combinations 
MC_rna_kraken_BC_dist_filt <- MC_rna_kraken_BC_dist %>% dplyr::filter(comparison %in% c(MC5_combs, MC19_combs))
...
Ran Humann3 on the mock communities to get gene family abundances because MC5 contains gut bacteria which will not be annotated by the skin gene catalog.

Pathway activity calculations for mock communities

```{r}
MC_activity_norm_dataset <- lapply(MC_library_metadata$LIBID, function(x){
  
  summary_df <- read_tsv(paste0("../data/mock_comm_RNA_out/",x, "_merged_decont_pathabundance.tsv"),
                         show_col_types = FALSE, skip =1, col_names = c("pathway",
                                                               "abundance")) %>% 
...
write_tsv(MC_activity_norm_dataset, file="../data/mock_comm_RNA_out/MC_humann_pathway_norm_dataset.tsv")
...
```{r}
MC_activity_mat <- pivot_wider(MC_activity_norm_dataset, id_cols=pathway, names_from=LIBID, 
                                          values_from=abundance) %>% as.data.frame()



MC_activity_mat[is.na(MC_activity_mat)] <- 0

rownames(MC_activity_mat) <- MC_activity_mat$pathway

MC_activity_mat <- MC_activity_mat %>% dplyr::select(-pathway) %>% as.matrix()

MC_activity_pearson_mat <- cor(MC_activity_mat, method="pearson")


#both upper and lower triangles represented here
MC_activity_pearson_df  <- reshape2::melt(MC_activity_pearson_mat, varnames = c("row", "col"))
      
MC_activity_pearson_df <- MC_activity_pearson_df  %>% dplyr::filter(col!=row)

#comparison for selection purposes
MC_activity_pearson_df$comparison <- paste0(MC_activity_pearson_df$row,"-", MC_activity_pearson_df$col)

MC_activity_pearson_df_filt <- MC_activity_pearson_df %>% dplyr::filter(comparison %in% c(MC5_combs, MC19_combs))

median(MC_activity_pearson_df_filt$value) #0.998



```
Three biological replicates per mock community.

Boxplot of Sorensen Similarity Indices (1 – Bray Curtis dissimilarity) computed from species level relative abundances for the cultured mock communities. 
Boxplot of Pearson correlation coefficient for gene-level TPMs from species of the cultured mock communities

```{r}

MC_BC_dist_fmt <- MC_rna_kraken_BC_dist_filt %>% dplyr::select(comparison, sorensen_similarity_species_level) %>%
  dplyr::rename(value = sorensen_similarity_species_level)

MC_BC_dist_fmt$metric <- "sorensen_similarity"

MC_activity_pearson_fmt <- MC_activity_pearson_df_filt %>% dplyr::select(comparison, value)

MC_activity_pearson_fmt$metric <- "Pearson_correlation_expressed_pathways"

MC_combined_metrics <- rbind(MC_BC_dist_fmt, MC_activity_pearson_fmt)


MC_combined_metrics$metric <- factor(MC_combined_metrics$metric, levels = c("sorensen_similarity",
                                                                     "Pearson_correlation_expressed_pathways"))



MC_metrics_boxplot <- ggplot(MC_combined_metrics,
       aes(x=metric, y=value)) + 
  geom_boxplot() + theme_classic() +
  ylab("Index or Coefficient")



ggsave(plot=MC_metrics_boxplot, filename="../plots/MC_metrics_boxplot.pdf")
